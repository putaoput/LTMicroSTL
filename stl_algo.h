...
namespace LT {
   ...
   //但是在核心代码上，插入排序只要一个赋值(移动)，但是冒泡排序需要三个（交换）
    ...
    template <class ForwardIter, class T>
    ForwardIter
        ubound_dispatch(ForwardIter _first, ForwardIter _last,
            const T& _value, forward_iterator_tag)
    {
        auto len = LT::distance(_first, _last);
        auto half = len;
        ForwardIter middle;
    ...
    template <class RandomIter, class T>
    RandomIter
        ubound_dispatch(RandomIter _first, RandomIter _last,
            const T& _value, random_access_iterator_tag)
    {
        auto len = _last - _first;
        auto half = len;
        RandomIter middle;
    ...
    template <class ForwardIter, class T>
    ForwardIter
        upper_bound(ForwardIter _first, ForwardIter _last, const T& _value)
    {
        return LT::ubound_dispatch(_first, _last, _value, iterator_category(_first));
    }
    ...
    ForwardIter
        ubound_dispatch(ForwardIter _first, ForwardIter _last,
            const T& _value, forward_iterator_tag, Compared comp)
    {
        auto len = LT::distance(_first, _last);
        auto half = len;
        ForwardIter middle;
    ...
    RandomIter
        ubound_dispatch(RandomIter _first, RandomIter _last,
            const T& _value, random_access_iterator_tag, Compared comp)
    {
        auto len = _last - _first;
        auto half = len;
        RandomIter middle;
    ...
    template <class ForwardIter, class T, class Compared>
    ForwardIter
        upper_bound(ForwardIter _first, ForwardIter _last, const T& _value, Compared comp)
    {
        return LT::ubound_dispatch(_first, _last, _value, iterator_category(_first), comp);
    }

    /*****************************************************************************************/
    // binary_search
    // 二分查找，如果在[_first, _last)内有等同于 _value 的元素，返回 true，否则返回 false
    /*****************************************************************************************/
    template <class ForwardIter, class T>
    bool binary_search(ForwardIter _first, ForwardIter _last, const T& _value)
    {
        auto i = LT::lower_bound(_first, _last, _value);
        return i != _last && !(_value < *i);
    }

    // 重载版本使用函数对象 comp 代替比较操作
    template <class ForwardIter, class T, class Compared>
    bool binary_search(ForwardIter _first, ForwardIter _last, const T& _value, Compared comp)
    {
        auto i = LT::lower_bound(_first, _last, _value);
        return i != _last && !comp(_value, *i);
    }
}
