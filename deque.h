...
namespace LT {
	...
	struct __deque_iterator
	{
		...
		__deque_iterator(const_iterator& _rhs) :cur_(_rhs.cur_), first_(_rhs.first_), last_(_rhs.last_), node_(_rhs.node_) {}
		...
		difference_type operator-(const self& _x)const {
			return static_cast<difference_type>(buffer_size() * (node_ - _x.node_ - 1) + cur_ - first_ + _x.last_ - _x.cur);
		}
		...
		void insert(iterator _pos, InputIterator _first, InputIterator _last)
{
	...
	protected:
		...
		inline T* __deque_allocator(size_type _mapSize = __deque_iterator::buffer_size()){
			//这个是按多少个T来分配大小
		...
		{
			LT::allocator<T>::deallocator(_ptr,sizeof(T) * _size);
		}
		...
		inline void __create_map_and_nodes(size_type _numElements) {
			size_type nodeNum = _numElements / __deque_iterator::buffer_size() + 1;
			...
			map_pointer nfinish = nstart + mapSize_;
			map_pointer cur;
			try {
				...
				for (; cur > nstart;) {
					--cur;
					data_dealloctor(cur);//rollback
				}
			}
			//start,cur,finish都是指向的node结点，而node结点内部的信息由迭代器维护。因此用set_node更新迭代器内部的三个指针
			start_.set_node(nstart);
			finish_.set_node(nfinish);
			start_.cur = start_.first_;
			finish_.cur = finish_.first_ + _numElements % __deque_iterator::buffer_size();
		...
		//--------------------------------------------迭代器的封装模板---------------------------------------------------
		//该模板可以使得deque内部的所有操作执行起来都在一块连续内存上执行一样。
		//这个地方是个人认为deque编写里面精巧的地方，在模板编程里面运用模板来减小代码量，也算是学以致用了
		//为什么不用迭代器自增一个一个的进行构造呢，因为对于大部分的数据都是基本数据类型，处理这类对象，一次处理一块连续内存会更快。
		...
		//------------------------------------------- 初始化----------------------------------------------------------------
		inline void __init(size_type _numElements, value_type _value = T()) 
		{
		...
		inline void __deallocate_all_mem(map_pointer _first, map_pointer _last) {
			...
			map_dealloctor(_first);//这一步会把每一个节点申请的内存空间析构掉。
			map_ = nullptr;
			start_.clear();
			finish_.clear();
			mapSize_ = 0;
	...
	//swap
	template<class T>
	void swap(deque<T>& _lhs, deque<T>& _rhs)
	{
		_lhs.swap(_rhs);
	}
	...
	//比较操作符
	template <class T>
	bool operator==(const deque<T>& _lhs, const deque<T>& _rhs)
	{
		return _lhs.size() == _rhs.size() && LT::equal(_lhs.begin(), _lhs.end(), _rhs.begin());
	}
