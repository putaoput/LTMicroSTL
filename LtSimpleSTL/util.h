...
namespace LT {
	...
	inline ForwardIter2 swap_range(ForwardIter1 first1, ForwardIter1 _last1, ForwardIter2 _first2)
	{
		for (; first1 != _last1; ++first1, (void) ++_first2)
			LT::swap(*first1, *_first2);
		return _first2;
	}
	...
	void __adjust(RandomIterator _itBegin, size_t _index, size_t _len, Comp _cmp) {
	...
	struct pair
	{
		...
		typedef T1  second_type;//首先是要提供给使用者萃取成员变量类型的接口，
		...
		pair& operator=(const pair&& _p) {
			first = move(_p.first);//注意默认是在LT命名域下，所以调用的是LT
			second = move(_p.second);
			return *this;
		}
	...
	//pair的一系列辅助函数：
	template<class T1, class T2>
	pair<T1, T2>  make_pair(const T1& _first, const T1& _second) {
		return pair<T1, T2>(forward(_first),forward(_second));
	}
